...
namespace Engine
{
  namespace Render
  {
    ...
    RenderDevice::RenderDevice(Engine::Core::Window& window) : window{ window } {
      // init vulkan lib and bridge between vulkan and the engine
      createInstance();
      // enable validation layer for debug builds. Vulkan does a little error checking
      setupDebugMessenger();
      // init a bridge between vulkan and a window vulkan draws into
      createSurface();
      // init a physical GPU
      pickPhysicalDevice();
      // describe features of the GPU for later use in the renderer
      createLogicalDevice();
      // create pool for command allocations for later use
      createCommandPool();
    }
    ...
    void RenderDevice::createInstance() {
      ...
      appInfo.pApplicationName = "Flyeng";
      appInfo.applicationVersion = VK_MAKE_VERSION(1, 0, 0);
      appInfo.pEngineName = "Flyeng";
